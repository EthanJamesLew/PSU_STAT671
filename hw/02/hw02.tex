...
\begin{aligned} K(x, y)&= y^T S_x y,
 \end{aligned}
\end{equation}
such that $S_x = x x^T$. Now, using Equation \ref{equ:frob}, evaluate the LHS of the reproducing property equation,
\begin{equation}
\begin{aligned}
...
\begin{equation}
\mathcal{Y}=\operatorname{span}\{K(x, \cdot) | x \in \mathcal{X}\}.
\end{equation}
